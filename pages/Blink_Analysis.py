...
<body>
    <div style="text-align: center;">
        ...
        <br><br>
        <button id="startBtn" style="padding: 10px 20px; font-size: 16px; background-color: #3498db; color: white; border: none; border-radius: 5px; cursor: pointer; margin: 5px;">Start Camera</button>
        <button id="captureBtn" style="padding: 10px 20px; font-size: 16px; background-color: #27ae60; color: white; border: none; border-radius: 5px; cursor: pointer; margin: 5px;" disabled>Capture & Upload Frames</button>
    ...
    <script>
        ...
        captureBtn.onclick = async () => {
            if (!stream) { status.textContent = 'Please start the camera first!'; return; }
            ...
            for (let i = 0; i < 120; i++) {
                ...
                if (i % 3 === 0) { previewCtx.drawImage(video, 0, 0, 320, 240); }
...
"""
    st.components.v1.html(html_code, height=780)
...
age_num = st.number_input("Age", min_value=1, max_value=123, value=25, step=1, key="age_input", help="Use the +/- buttons or type your age (1-123)")
st.write("---")
